...
@Test func testVersionFromPacked() async throws {
    ...
    let version = Version(packed: packed)
...
@Test func testFFmpegVersions() async throws {
    ...
    let avformat = FFmpeg.Version.avformat
    #expect(avformat.major > 0)

    let avutil = FFmpeg.Version.avutil
    #expect(avutil.major > 0)

    let swscale = FFmpeg.Version.swscale
    #expect(swscale.major > 0)

    let swresample = FFmpeg.Version.swresample
